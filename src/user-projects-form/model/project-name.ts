import { combine, createEvent, createStore, forward, guard } from 'effector';
import { createFormField } from '../../lib/create-form-field';
import { getProjectsDataFx } from './main';

export const $projectNames = createStore<string[]>([])
const projectNameAdded = createEvent<string>()
...
$newProjectName
  .reset(projectNameAdded)
const $hasNewProjectNameInProjects = combine(
  $projectNames,
  $newProjectName,
...
const $isNewProjectNameValid = combine(
  newProjectNameField.$innerError,
  $hasNewProjectNameInProjects,
  (innerError, hasTheSameName) => !innerError && !hasTheSameName
)
export const $newProjectNameError = combine(
  newProjectNameField.$isFieldTouched,
  newProjectNameField.$error,
  $hasNewProjectNameInProjects,
  (isTouched, innerError, hasTheSameName) => {
    if (!isTouched) return ''
    if (innerError) return innerError
    if (hasTheSameName) return 'Already exist'
    return ''
  }
)
...
export const projectNameRemoved = createEvent<string>()

$projectNames
  .on(projectNameAdded, (names, newName) => [...names, newName])
  .on(projectNameRemoved, (names, nameToRemove) => {
